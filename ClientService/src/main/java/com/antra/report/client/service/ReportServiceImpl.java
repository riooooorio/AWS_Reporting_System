...
public class ReportServiceImpl implements ReportService {
    ...
    private ReportRequestEntity persistToLocal(ReportRequest request) {
        return persistToLocal("Req-"+ UUID.randomUUID().toString(), request);
    }

    private ReportRequestEntity persistToLocal(String reqId, ReportRequest request) {
    ...
    private void sendDirectRequests(ReportRequest request) {
        ...
        CompletableFuture<Void> excelRequestFuture = CompletableFuture.runAsync(() -> {
            ExcelResponse excelResponse = new ExcelResponse();
            try {
                excelResponse = rs
                    .postForEntity("http://localhost:8888/excel", request, ExcelResponse.class)
                    .getBody();
            } catch (Exception e) {
        ...
        CompletableFuture<Void> pdfRequestFuture = CompletableFuture.runAsync(() -> {
            PDFResponse pdfResponse = new PDFResponse();
            try {
                pdfResponse = rs
                    .postForEntity("http://localhost:9999/pdf", request, PDFResponse.class)
                    .getBody();
            } catch (Exception e) {
        ...
        CompletableFuture.allOf(excelRequestFuture, pdfRequestFuture).join();
    }
    ...
    public ReportVO updateReport(String reqId, ReportRequest request) {
        if (!reportRequestRepo.findById(request.getReqId()).isPresent()) {
            throw new IllegalStateException("ReqId for update not found");
        }
        persistToLocal(reqId, request);
        sendDirectRequests(request);
        return new ReportVO(reportRequestRepo.findById(request.getReqId()).orElseThrow());
    }
...
//            InputStream is = restTemplate.execute(, HttpMethod.GET, null, ClientHttpResponse::getBody, fileId);
    ...
    @Override
    public boolean deleteReport(String reqId) {
        if (reportRequestRepo.findById(reqId).isPresent()) {
