...
public class ReportController {
    ...
    public ResponseEntity<GeneralResponse> deleteReport(@PathVariable String reqId) {
        if (reportService.deleteReport(reqId)) {
            return ResponseEntity.ok(new GeneralResponse());
        }
        return new ResponseEntity<>(new ErrorResponse(HttpStatus.NOT_FOUND, "reqId not found"), HttpStatus.BAD_REQUEST);
    }

    @PutMapping("/report/{reqId}")
    public ResponseEntity<GeneralResponse> updateReport(
        @PathVariable String reqId, @RequestBody @Validated ReportRequest request
    ) {
        reportService.updateReport(reqId, request);
        return ResponseEntity.ok(new GeneralResponse());
    }
